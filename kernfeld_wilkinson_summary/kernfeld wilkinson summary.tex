...
%Note to self: this is the nicest simple test from the 24 simple tests.
%\begin{figure}[h!]
%\begin{center}
...
%Bandwidth selection is important, and there is no theory to guide it here, but the state of the art is to take an out-of-the-box rule and divide the resulting bandwidth by ten \cite{wilk_email}. 
...
\begin{figure}[h!]
\begin{center}
\includegraphics[height=2in,width=2in]{geweke_qq/obs.png}
\includegraphics[height=2in,width=2in]{geweke_qq/params.png}
\includegraphics[height=2in,width=2in]{geweke_qq/state.png}
\caption{ \label{fig:geweke} Results from a Geweke test of the pMCMC\_julia.jl module. Each pane gives a Q-Q plot comparing observations (left), parameters (center), or hidden states at time 1 (right) from  (iid generative vs. MCMC).  Each plot also shows $y=x$ for reference.}
\end{center}
\end{figure}

\item Unlike MCMC, any implementation of the Gillespie algorithm can be compared against analytical results for simple systems. For example, a single-model, single-reaction system with $n_0$ molecules decaying at rate $k$ can be repeated many times, and after $t$ seconds, the average number of molecules left will be $n_0\exp(-kt)$. Figure \label{fig:gillespie_test} illustrates such a comparison (left pane), along with a typical simulation from the {\it B. subtilis} motility regulation model from my experiments (center) and W10's (right).
...
%\item During the project, my UW mentors recommended trying experiments with simple systems where the algorithm's behavior can be predicted. I have implemented one test where the posterior distribution can be calculated. Suppose the underlying chemical system has no reactions and one molecule, with measurement error that follows a binomial distribution with count 40, probability 0.5 (centered to have mean zero). If the prior is a discrete uniform on zero to 500 and the true value is not below 20 or above 480, then the posterior should center around the true value with a mass function proportional to a binomial PMF taken to the power of the number of observations. In a test of this form, the posterior samples center around the true value.
%
%I implemented another test where the 
\end{itemize}

\subsection{Sample impoverishment}
\label{sec:samp_impov}
Sequential Monte Carlo suffers from a problem termed sample impoverishment, which means that eventually most samples are duplicates of a common ancestor and few unique values remain. Plots of results even with 1,000,000 samples seem to display this issue. To give an example, Figure \ref{fig:main_results} shows the posterior distribution over two unknown rates from a replicate of one of W10's experiments. %In the synthetic data, twenty-four observations occurred at intervals of 300 seconds, with a noise sd of 10.0. There were 1,000,000 particles, with true rates and log-uniform priors matching Wilkinson's. 
